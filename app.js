...
function updateFocusMode(diffToOnAir) {
  const shouldFocus =
    (targets.onAir !== null && diffToOnAir <= 60) ||
    onAirCountdownEl.textContent === "I sändning";
...
function openPicker(title, key) {
  ...
  if (targets[key] !== null) {
    safe = ((targets[key] % 86400) + 86400) % 86400;
  } else {
    const now = new Date();
    safe = now.getHours() * 3600 + now.getMinutes() * 60;
  }

  hSel.value = String(Math.floor(safe / 3600));
  mSel.value = String(Math.floor((safe % 3600) / 60));
  sSel.value = String(targets[key] !== null ? safe % 60 : 0);
